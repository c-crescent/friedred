...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (ABILITY_SNOW_CLOAK && gBattleWeather & B_WEATHER_HAIL)
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_NO_GUARD || gBattleMons[gBattlerTarget].ability == ABILITY_NO_GUARD)
            calc = (calc * 1000) / 100; // No Guard can't miss, but the game still does the calculation and checks if the random number is less than the result. This makes it so that No Guard moves have a 1000/100 = 10x boost to their accuracy, which means they will always hit.
...
const u16 levelCapFlags[NUM_HARD_CAPS] = {
...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) >= GetCurrentLevelCap() || gSaveBlock1Ptr->keyFlags.expMod == 0 || FlagGet(FLAG_MASTER_TRAINER_BATTLE))
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
    ...
    do
    {
        ...
        case MOVEEND_CHOICE_MOVE: // update choice band move
            if (gHitMarker & HITMARKER_OBEYS
             && (holdEffectAtk == HOLD_EFFECT_CHOICE_BAND || HOLD_EFFECT_CHOICE_SPECS)
...
static void Cmd_getmoneyreward(void)
{
    ...
    if (gBattleOutcome == B_OUTCOME_WON)
    {
        ...
        else
        {
            ...
            moneyReward = 6 * lastMonLevel * gBattleStruct->moneyMultiplier * (gBattleTypeFlags & BATTLE_TYPE_DOUBLE ? 2 : 1) * gTrainerMoneyTable[i].value;
        }
